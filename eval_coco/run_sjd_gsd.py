"""
Evaluation-dataset inference with SJD / GSD / AR for Janus.

Usage examples
--------------
# AR baseline, COCO val, all 5000 samples
python eval_coco/run_sjd_gsd.py --method ar --split val --save_dir generated/ar_coco_val

# SJD, first 1000 COCO val samples, save decoded images
python eval_coco/run_sjd_gsd.py --method sjd --end 1000 --do_decode_image \
    --save_dir generated/sjd_coco_val

# GSD, custom prompt JSON (key "text"), save decoded images + raw tokens
python eval_coco/run_sjd_gsd.py --method gsd --prompt_path my_prompts.json \
    --json_key text --id_key id --do_decode_image --do_save_token \
    --save_dir generated/gsd_custom

# GSD with non-default cluster params
python eval_coco/run_sjd_gsd.py --method gsd --gsd_G 20 --gsd_p_thr 0.2 \
    --save_dir generated/gsd_G20

# Distributed: shard 0-500 on GPU 0, 500-1000 on GPU 1
python eval_coco/run_sjd_gsd.py --method sjd --begin 0   --end 500  --name_offset 0
python eval_coco/run_sjd_gsd.py --method sjd --begin 500 --end 1000 --name_offset 500
"""
...
# Make the repo root importable when invoked from a subdirectory.
...
def _run_ar(
    ...
    """Baseline AR identical to generation_inference.py."""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Run SJD / GSD / AR inference over a dataset of prompts."
    )

    # ── dataset ───────────────────────────────────────────────────────────────
    p.add_argument("--prompt_path",  type=str,  default=None,
                   help="Path to JSON prompt file.  Defaults to COCO val/train.")
    p.add_argument("--dataset_name", type=str,  default="COCO",
                   choices=["COCO", "laion", "midjourney", "custom"],
                   help="Used to choose a default prompt path / id key.")
    p.add_argument("--split",        type=str,  default="val",
                   choices=["train", "val"])
    p.add_argument("--json_key",     type=str,  default="caption",
                   help="Key inside each JSON record that holds the text prompt.")
    p.add_argument("--id_key",       type=str,  default=None,
                   help="Key for the image ID used in output filenames.  "
                        "Auto-detected from dataset_name when omitted.")
    p.add_argument("--begin",        type=int,  default=0)
    p.add_argument("--end",          type=int,  default=None,
                   help="Exclusive end index (None = whole file).")
    ...
    p.add_argument("--method",       type=str,  default="sjd",
                   choices=["ar", "sjd", "gsd"])
    p.add_argument("--jacobi_window", type=int, default=16,
                   help="Jacobi window size (SJD & GSD).")
    p.add_argument("--max_iter",     type=int,  default=20,
                   help="Max Jacobi iterations per window (SJD & GSD).")
    p.add_argument("--gsd_G",        type=int,  default=14,
                   help="GSD cluster half-width G.")
    p.add_argument("--gsd_p_thr",    type=float, default=0.15,
                   help="GSD probability-similarity threshold.")
    p.add_argument("--gsd_d_thr",    type=float, default=0.5,
                   help="GSD embedding-distance threshold (normalised).")
    ...
    p.add_argument("--do_decode_image", action="store_true",
                   help="Decode VQ tokens to JPEG images.")
    p.add_argument("--do_save_token",   action="store_true",
                   help="Save raw token tensors (.pt files).")
...
def main():
    ...
    if prompt_path is None:
        raise ValueError(f"Cannot resolve prompt path for dataset '{args.dataset_name}'. "
    ...
    if args.save_dir is None:
        args.save_dir = os.path.join(
            "generated",
            f"{args.method}_{args.dataset_name}_{args.split}"
            f"_{args.begin}-{args.end or 'end'}"
        )
    ...
    # ── per-method generation function ───────────────────────────────────────
    ...
    for offset, item in tqdm(enumerate(subset), total=len(subset), desc=args.method.upper()):
        ...
        # ── optionally decode to image ────────────────────────────────────────
        if args.do_decode_image:
            arr = tokens_to_image(model, tokens, args.img_size, args.patch_size)
            save_image(arr, os.path.join(args.save_dir, f"generated_{img_id}.jpg"))

        # ── optionally save token tensor ──────────────────────────────────────
        if args.do_save_token:
            save_path = Path(args.save_dir) / f"token_sample_{global_idx + args.name_offset}.pt"
            torch.save({"tokens": tokens.cpu()}, save_path)
    ...
    print(f"\n{'─'*55}")
    print(f"  Method          : {args.method.upper()}")
    print(f"  Samples         : {total_valid}")
    print(f"  Avg time/image  : {total_time/n:.3f}s")
    print(f"  Avg fwd/image   : {agg_fwd/n:.1f}")
    print(f"  Avg tok/fwd     : {image_token_num / (agg_fwd/n):.3f}")
    if args.method != "ar":
        print(f"  Accept rate     : {ar:.3f}")
    print(f"{'─'*55}")
    print(f"Done. Saved {total_valid} samples to {args.save_dir}")
